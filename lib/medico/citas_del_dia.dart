...
class _CitaCardState extends State<CitaCard>
  ...
  Future<void> cambiarEstado(String nuevo) async {
    ...
    if (user != null) {
      final doc = await FirebaseFirestore.instance
          ...
          .doc(user.uid)
          .get();
      nombrePersonal = doc.data()?['nombre'] ?? 'Desconocido';
    }
      ...
      builder: (_) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _detalle('Paciente', data['paciente']),
              _detalle('Fecha', data['fecha']),
              _detalle('Hora', data['hora']),
              _detalle('Motivo', data['motivo']),
              _detalle('Triaje', data['triaje']),
              _detalle('Diagnóstico', data['diagnostico']),
              _detalle('Tratamiento', data['tratamiento']),
              _detalle('Observaciones', data['observaciones']),
              _detalle('Estado', estado),
              _detalle('Llevado por', data['modificado_por']),
            ],
          ),
        ),
        actions: [
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(valor ?? '-', style: const TextStyle(color: darkBlue)),
          ),
        ],
      ),
    );
  ...
  Future<void> mostrarDialogoMateriales(BuildContext context) async {
    final materialesSnapshot = await FirebaseFirestore.instance
        .collection('materiales')
        .get();

    final materiales = materialesSnapshot.docs;
    final materialesSeleccionados = <String, int>{};
            ...
            return AlertDialog(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 20),
                      _campoTexto('Diagnóstico', diagnosticoController),
                      _campoTexto('Tratamiento', tratamientoController),
                      _campoTexto('Observaciones', observacionesController),
                    ],
                  ),
                ),
              ),
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    try {
                      final usados = materialesSeleccionados.entries
                          .where((e) => e.value > 0)
                          .toList();
                          ...
                          SnackBar(
                            content: const Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Text('Debes ingresar información.'),
                              ],
                            ),
                      ...
                      for (final entry in usados) {
                        final matDoc = materiales.firstWhere(
                          (doc) => doc['nombre'] == entry.key,
                        );
                        final nuevoStock = (matDoc['stock'] ?? 0) - entry.value;
                      ...
                      await FirebaseFirestore.instance
                          ...
                          .update({
                            ...
                            'tratamiento': tratamientoController.text.trim(),
